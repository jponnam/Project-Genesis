...
def test_run_emits_lifecycle_and_tick_events() -> None:
    ...
    assert types.count(TechnologyCreated.__name__) == 37
    assert types.count(InnovationCreated.__name__) == 37
    ...
    assert types[53] == InnovationCreated.__name__
    assert types[90] == AgentSpawned.__name__
    ...
    assert len(result.world.technologies) == 37
    assert len(result.world.research_progress) == 36
    assert len(result.world.innovations) == 37
...
def test_laws_observed_each_tick_including_start() -> None:
    ...
    assert all(event.active_curriculum_count == 0 for event in observed)
    ...
    assert all(event.active_quarantine_count == 0 for event in observed)
    assert all(event.active_building_codes_count == 0 for event in observed)
    assert all(event.active_zoning_count == 0 for event in observed)
    assert all(event.active_passage_count == 0 for event in observed)
    assert all(event.active_customs_count == 0 for event in observed)
    assert all(event.active_land_tenure_count == 0 for event in observed)
    assert all(event.active_labor_count == 0 for event in observed)
    assert all(event.active_sumptuary_count == 0 for event in observed)
    assert all(event.active_mineral_rights_count == 0 for event in observed)
    assert all(event.active_safety_codes_count == 0 for event in observed)
    assert all(event.active_timber_rights_count == 0 for event in observed)
    ...
    assert all(event.active_crystal_codes_count == 0 for event in observed)
...
def test_institutions_observed_each_tick_including_start() -> None:
    ...
    assert all(event.active_workshop_count == 0 for event in observed)
    assert all(event.active_mason_count == 0 for event in observed)
    assert all(event.active_architect_count == 0 for event in observed)
    assert all(event.active_caravan_count == 0 for event in observed)
    assert all(event.active_merchant_count == 0 for event in observed)
    assert all(event.active_cartographer_count == 0 for event in observed)
    assert all(event.active_granary_count == 0 for event in observed)
    assert all(event.active_husbandman_count == 0 for event in observed)
    ...
    assert all(event.active_woodcutter_count == 0 for event in observed)
    assert all(event.active_joiner_count == 0 for event in observed)
    assert all(event.active_carver_count == 0 for event in observed)
    assert all(event.active_potter_count == 0 for event in observed)
    assert all(event.active_glazer_count == 0 for event in observed)
    assert all(event.active_tilewright_count == 0 for event in observed)
    assert all(event.active_glassblower_count == 0 for event in observed)
    assert all(event.active_lensmaker_count == 0 for event in observed)
    assert all(event.active_optician_count == 0 for event in observed)
...
def test_technologies_observed_each_tick_including_start() -> None:
    ...
    assert observed[0].tick.value == 0
    assert observed[-1].tick.value == 3
    assert all(event.technology_count == 37 for event in observed)
    ...
    assert all(event.discovered_mathematics_count == 0 for event in observed)
    assert all(event.discovered_astronomy_count == 0 for event in observed)
    assert all(event.discovered_philosophy_count == 0 for event in observed)
    assert all(event.discovered_logic_count == 0 for event in observed)
    assert all(event.discovered_rhetoric_count == 0 for event in observed)
    assert all(event.discovered_medicine_count == 0 for event in observed)
    assert all(event.discovered_anatomy_count == 0 for event in observed)
    assert all(event.discovered_hygiene_count == 0 for event in observed)
    ...
    assert all(event.discovered_cartography_count == 0 for event in observed)
    assert all(event.discovered_seafaring_count == 0 for event in observed)
    assert all(event.discovered_agriculture_count == 0 for event in observed)
    assert all(event.discovered_crop_rotation_count == 0 for event in observed)
    assert all(event.discovered_forestry_count == 0 for event in observed)
    assert all(event.discovered_textiles_count == 0 for event in observed)
    assert all(event.discovered_dyeing_count == 0 for event in observed)
    assert all(event.discovered_tanning_count == 0 for event in observed)
    assert all(event.discovered_mining_count == 0 for event in observed)
    assert all(event.discovered_smithing_count == 0 for event in observed)
    assert all(event.discovered_toolmaking_count == 0 for event in observed)
    assert all(event.discovered_carpentry_count == 0 for event in observed)
    assert all(event.discovered_joinery_count == 0 for event in observed)
    assert all(event.discovered_cabinetry_count == 0 for event in observed)
    assert all(event.discovered_ceramics_count == 0 for event in observed)
    assert all(event.discovered_glazing_count == 0 for event in observed)
    assert all(event.discovered_porcelain_count == 0 for event in observed)
    assert all(event.discovered_glassmaking_count == 0 for event in observed)
    assert all(event.discovered_optics_count == 0 for event in observed)
    assert all(event.discovered_crystal_count == 0 for event in observed)
    infra_indexes = [
        i
        for i, event in enumerate(result.events)
        if isinstance(event, InfrastructuresObserved)
...
def test_research_observed_each_tick_including_start() -> None:
    ...
    assert all(event.progress_count == 36 for event in observed)
    tech_indexes = [
        i
        for i, event in enumerate(result.events)
        if isinstance(event, TechnologiesObserved)
...
def test_innovations_observed_each_tick_including_start() -> None:
    ...
    assert observed[0].tick.value == 0
    assert observed[-1].tick.value == 3
    assert all(event.innovation_count == 37 for event in observed)
    ...
    assert all(event.active_abacus_count == 0 for event in observed)
    assert all(event.active_star_chart_count == 0 for event in observed)
    assert all(event.active_dialectic_count == 0 for event in observed)
    assert all(event.active_syllogism_count == 0 for event in observed)
    assert all(event.active_oration_count == 0 for event in observed)
    assert all(event.active_remedy_count == 0 for event in observed)
    assert all(event.active_dissection_count == 0 for event in observed)
    assert all(event.active_asepsis_count == 0 for event in observed)
    ...
    assert all(event.active_map_count == 0 for event in observed)
    assert all(event.active_sail_count == 0 for event in observed)
    assert all(event.active_plow_count == 0 for event in observed)
    assert all(event.active_fallow_count == 0 for event in observed)
    assert all(event.active_coppice_count == 0 for event in observed)
    assert all(event.active_loom_count == 0 for event in observed)
    assert all(event.active_mordant_count == 0 for event in observed)
    assert all(event.active_tannery_count == 0 for event in observed)
    assert all(event.active_pickaxe_count == 0 for event in observed)
    assert all(event.active_bellows_count == 0 for event in observed)
    assert all(event.active_lathe_count == 0 for event in observed)
    assert all(event.active_sawmill_count == 0 for event in observed)
    assert all(event.active_plane_count == 0 for event in observed)
    assert all(event.active_dovetail_count == 0 for event in observed)
    assert all(event.active_kiln_count == 0 for event in observed)
    assert all(event.active_glaze_count == 0 for event in observed)
    assert all(event.active_kaolin_count == 0 for event in observed)
    assert all(event.active_blowpipe_count == 0 for event in observed)
    assert all(event.active_lens_count == 0 for event in observed)
    assert all(event.active_facet_count == 0 for event in observed)
    research_indexes = [
        i
        for i, event in enumerate(result.events)
        if isinstance(event, ResearchObserved)
...
def test_knowledge_observed_each_tick_including_start() -> None:
    ...
    assert all(event.mathematics_knower_count == 0 for event in observed)
    assert all(event.astronomy_knower_count == 0 for event in observed)
    assert all(event.philosophy_knower_count == 0 for event in observed)
    assert all(event.logic_knower_count == 0 for event in observed)
    assert all(event.rhetoric_knower_count == 0 for event in observed)
    assert all(event.medicine_knower_count == 0 for event in observed)
    assert all(event.anatomy_knower_count == 0 for event in observed)
    assert all(event.hygiene_knower_count == 0 for event in observed)
    ...
    assert all(event.cartography_knower_count == 0 for event in observed)
    assert all(event.seafaring_knower_count == 0 for event in observed)
    assert all(event.agriculture_knower_count == 0 for event in observed)
    assert all(event.crop_rotation_knower_count == 0 for event in observed)
    assert all(event.forestry_knower_count == 0 for event in observed)
    assert all(event.textiles_knower_count == 0 for event in observed)
    assert all(event.dyeing_knower_count == 0 for event in observed)
    assert all(event.tanning_knower_count == 0 for event in observed)
    assert all(event.mining_knower_count == 0 for event in observed)
    assert all(event.smithing_knower_count == 0 for event in observed)
    assert all(event.toolmaking_knower_count == 0 for event in observed)
    assert all(event.carpentry_knower_count == 0 for event in observed)
    assert all(event.joinery_knower_count == 0 for event in observed)
    assert all(event.cabinetry_knower_count == 0 for event in observed)
    assert all(event.ceramics_knower_count == 0 for event in observed)
    assert all(event.glazing_knower_count == 0 for event in observed)
    assert all(event.porcelain_knower_count == 0 for event in observed)
    assert all(event.glassmaking_knower_count == 0 for event in observed)
    assert all(event.optics_knower_count == 0 for event in observed)
    assert all(event.crystal_knower_count == 0 for event in observed)
    innovation_indexes = [
        i
        for i, event in enumerate(result.events)
        if isinstance(event, InnovationsObserved)
