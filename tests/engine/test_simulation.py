...
def test_run_emits_lifecycle_and_tick_events() -> None:
    ...
    assert types.count(TechnologyCreated.__name__) == 13
    assert types.count(InnovationCreated.__name__) == 13
    ...
    assert types[29] == InnovationCreated.__name__
    assert types[42] == AgentSpawned.__name__
    ...
    assert len(result.world.technologies) == 13
    assert len(result.world.research_progress) == 12
    assert len(result.world.innovations) == 13
...
def test_technologies_observed_each_tick_including_start() -> None:
    ...
    assert observed[0].tick.value == 0
    assert observed[-1].tick.value == 3
    assert all(event.technology_count == 13 for event in observed)
...
def test_research_observed_each_tick_including_start() -> None:
    ...
    assert all(event.progress_count == 12 for event in observed)
    tech_indexes = [
        i
        for i, event in enumerate(result.events)
        if isinstance(event, TechnologiesObserved)
...
def test_innovations_observed_each_tick_including_start() -> None:
    ...
    assert observed[0].tick.value == 0
    assert observed[-1].tick.value == 3
    assert all(event.innovation_count == 13 for event in observed)
