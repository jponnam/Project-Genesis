...
def test_run_emits_lifecycle_and_tick_events() -> None:
    ...
    assert types.count(TechnologyCreated.__name__) == 18
    assert types.count(InnovationCreated.__name__) == 18
    ...
    assert types[34] == InnovationCreated.__name__
    assert types[52] == AgentSpawned.__name__
    ...
    assert len(result.world.technologies) == 18
    assert len(result.world.research_progress) == 17
    assert len(result.world.innovations) == 18
...
def test_technologies_observed_each_tick_including_start() -> None:
    ...
    assert observed[0].tick.value == 0
    assert observed[-1].tick.value == 3
    assert all(event.technology_count == 18 for event in observed)
...
def test_research_observed_each_tick_including_start() -> None:
    ...
    assert all(event.progress_count == 17 for event in observed)
    tech_indexes = [
        i
        for i, event in enumerate(result.events)
        if isinstance(event, TechnologiesObserved)
...
def test_innovations_observed_each_tick_including_start() -> None:
    ...
    assert observed[0].tick.value == 0
    assert observed[-1].tick.value == 3
    assert all(event.innovation_count == 18 for event in observed)
