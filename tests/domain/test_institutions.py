...
def test_default_institutions_seed_camp_council() -> None:
    ...
    assert all(
        item.kind is not InstitutionKind.SMITH for item in default_institutions()
    )
    assert all(
        item.kind is not InstitutionKind.CARVER for item in default_institutions()
    )
    assert all(
        item.kind is not InstitutionKind.POTTER for item in default_institutions()
    )
    assert all(
        item.kind is not InstitutionKind.GLAZER for item in default_institutions()
    )
    assert all(
    ...
    assert all(
        item.kind is not InstitutionKind.LENSMAKER for item in default_institutions()
    )
    assert all(
        item.kind is not InstitutionKind.OPTICIAN for item in default_institutions()
    )
...
def test_create_smith_alongside_other_kinds() -> None:
    """Smiths coexist with other kinds; census counts each kind."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        institutions=(
            Institution.create(0, 0, 0, "Council", InstitutionKind.COUNCIL),
    ...
    with_smith = create_institution(
        world,
        Institution.create(25, 0, 0, "Camp Smith", InstitutionKind.SMITH),
    )
    assert with_smith is not None
    assert with_smith.institutions[25].kind is InstitutionKind.SMITH
    snap = census_institutions(with_smith)
    assert snap.active_council_count == 1
    assert snap.active_tailor_count == 1
    assert snap.active_smelter_count == 1
    assert snap.active_smith_count == 1
    assert snap.active_count == 26
    assert (
        create_institution(
            with_smith,
            Institution.create(26, 0, 0, "Second Smith", InstitutionKind.SMITH),
        )
        is None
    )


def test_create_woodcutter_alongside_other_kinds() -> None:
    """Woodcutters coexist with other kinds; census counts each kind."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        institutions=(
            Institution.create(0, 0, 0, "Council", InstitutionKind.COUNCIL),
    ...
    with_woodcutter = create_institution(
        world,
        Institution.create(26, 0, 0, "Camp Woodcutter", InstitutionKind.WOODCUTTER),
    )
    assert with_woodcutter is not None
    assert with_woodcutter.institutions[26].kind is InstitutionKind.WOODCUTTER
    snap = census_institutions(with_woodcutter)
    assert snap.active_council_count == 1
    assert snap.active_smith_count == 1
    assert snap.active_woodcutter_count == 1
    assert snap.active_count == 27
    assert (
        create_institution(
            with_woodcutter,
            Institution.create(
                27, 0, 0, "Second Woodcutter", InstitutionKind.WOODCUTTER
            ),
        )
        is None
    )
...
def test_create_joiner_alongside_other_kinds() -> None:
    """Joiners coexist with other kinds; census counts each kind."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        institutions=(
            Institution.create(0, 0, 0, "Council", InstitutionKind.COUNCIL),
    ...
    assert with_joiner is not None
    assert with_joiner.institutions[27].kind is InstitutionKind.JOINER
    snap = census_institutions(with_joiner)
    assert snap.active_council_count == 1
    assert snap.active_woodcutter_count == 1
    assert snap.active_joiner_count == 1
    assert snap.active_count == 28
    assert (
        create_institution(
            with_joiner,
            Institution.create(28, 0, 0, "Second Joiner", InstitutionKind.JOINER),
        )
        is None
    )


def test_create_carver_alongside_other_kinds() -> None:
    """Carvers coexist with other kinds; census counts each kind."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        institutions=(
            Institution.create(0, 0, 0, "Council", InstitutionKind.COUNCIL),
    ...
    with_carver = create_institution(
        world,
        Institution.create(28, 0, 0, "Camp Carver", InstitutionKind.CARVER),
    )
    assert with_carver is not None
    assert with_carver.institutions[28].kind is InstitutionKind.CARVER
    snap = census_institutions(with_carver)
    assert snap.active_council_count == 1
    assert snap.active_joiner_count == 1
    assert snap.active_carver_count == 1
    assert snap.active_count == 29
    assert (
        create_institution(
            with_carver,
            Institution.create(29, 0, 0, "Second Carver", InstitutionKind.CARVER),
        )
        is None
    )
...
def test_create_potter_alongside_other_kinds() -> None:
    """Potters coexist with other kinds; census counts each kind."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        institutions=(
            Institution.create(0, 0, 0, "Council", InstitutionKind.COUNCIL),
    ...
    with_potter = create_institution(
        world,
        Institution.create(29, 0, 0, "Camp Potter", InstitutionKind.POTTER),
    )
    assert with_potter is not None
    assert with_potter.institutions[29].kind is InstitutionKind.POTTER
    snap = census_institutions(with_potter)
    assert snap.active_council_count == 1
    assert snap.active_joiner_count == 1
    assert snap.active_carver_count == 1
    assert snap.active_potter_count == 1
    assert snap.active_count == 30
    assert (
        create_institution(
            with_potter,
            Institution.create(30, 0, 0, "Second Potter", InstitutionKind.POTTER),
        )
        is None
    )
...
def test_create_glazer_alongside_other_kinds() -> None:
    """Glazers coexist with other kinds; census counts each kind."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        institutions=(
            Institution.create(0, 0, 0, "Council", InstitutionKind.COUNCIL),
    ...
    with_glazer = create_institution(
        world,
        Institution.create(30, 0, 0, "Camp Glazer", InstitutionKind.GLAZER),
    )
    assert with_glazer is not None
    assert with_glazer.institutions[30].kind is InstitutionKind.GLAZER
    snap = census_institutions(with_glazer)
    assert snap.active_council_count == 1
    assert snap.active_potter_count == 1
    assert snap.active_glazer_count == 1
    assert snap.active_count == 31
    assert (
        create_institution(
            with_glazer,
            Institution.create(31, 0, 0, "Second Glazer", InstitutionKind.GLAZER),
        )
        is None
    )
...
def test_create_tilewright_alongside_other_kinds() -> None:
    """Tilewrights coexist with other kinds; census counts each kind."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        institutions=(
            Institution.create(0, 0, 0, "Council", InstitutionKind.COUNCIL),
    ...
    with_tilewright = create_institution(
        world,
        Institution.create(31, 0, 0, "Camp Tilewright", InstitutionKind.TILEWRIGHT),
    )
    assert with_tilewright is not None
    assert with_tilewright.institutions[31].kind is InstitutionKind.TILEWRIGHT
    snap = census_institutions(with_tilewright)
    assert snap.active_council_count == 1
    assert snap.active_glazer_count == 1
    assert snap.active_tilewright_count == 1
    assert snap.active_count == 32
    assert (
        create_institution(
            with_tilewright,
            Institution.create(
                32, 0, 0, "Second Tilewright", InstitutionKind.TILEWRIGHT
            ),
        )
        is None
    )
...
def test_create_lensmaker_alongside_other_kinds() -> None:
    """Lensk makers coexist with other kinds; census counts each kind."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        institutions=(
            Institution.create(0, 0, 0, "Council", InstitutionKind.COUNCIL),
            ...
            Institution.create(31, 0, 0, "Camp Tilewright", InstitutionKind.TILEWRIGHT),
            Institution.create(
                32, 0, 0, "Camp Glassblower", InstitutionKind.GLASSBLOWER
            ),
        ),
    )
    with_lensmaker = create_institution(
        world,
        Institution.create(33, 0, 0, "Camp Lensmaker", InstitutionKind.LENSMAKER),
    )
    assert with_lensmaker is not None
    assert with_lensmaker.institutions[33].kind is InstitutionKind.LENSMAKER
    snap = census_institutions(with_lensmaker)
    assert snap.active_council_count == 1
    assert snap.active_glassblower_count == 1
    assert snap.active_lensmaker_count == 1
    assert snap.active_count == 34
    assert (
        create_institution(
            with_lensmaker,
            Institution.create(
...
def test_create_optician_alongside_other_kinds() -> None:
    """Opticians coexist with other kinds; census counts each kind."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        institutions=(
            Institution.create(0, 0, 0, "Council", InstitutionKind.COUNCIL),
    ...
    with_optician = create_institution(
        world,
        Institution.create(34, 0, 0, "Camp Optician", InstitutionKind.OPTICIAN),
    )
    assert with_optician is not None
    assert with_optician.institutions[34].kind is InstitutionKind.OPTICIAN
    snap = census_institutions(with_optician)
    assert snap.active_council_count == 1
    assert snap.active_lensmaker_count == 1
    assert snap.active_optician_count == 1
    assert snap.active_count == 35
    assert (
        create_institution(
            with_optician,
            Institution.create(
...
def test_census_institutions_counts() -> None:
    ...
    assert snap.active_smith_count == 0
    assert snap.active_woodcutter_count == 0
    assert snap.active_joiner_count == 0
    assert snap.active_carver_count == 0
    ...
    assert snap.active_lensmaker_count == 0
    assert snap.active_optician_count == 0
