...
def test_law_created_and_observed_round_trips() -> None:
    ...
    observed = LawsObserved(
        ...
        active_land_tenure_count=0,
        active_conservation_count=0,
        active_labor_count=0,
        active_sumptuary_count=0,
        active_mineral_rights_count=0,
        active_safety_codes_count=0,
        active_timber_rights_count=0,
        active_forest_management_count=0,
        active_firing_codes_count=0,
        active_clay_codes_count=0,
        active_annealing_codes_count=0,
        active_crystal_codes_count=0,
    )
    restored = event_from_record(observed.to_record())
    ...
    assert restored.active_quarantine_count == 0
    ...
    assert restored.active_land_tenure_count == 0
    assert restored.active_conservation_count == 0
    assert restored.active_labor_count == 0
    assert restored.active_sumptuary_count == 0
    assert restored.active_mineral_rights_count == 0
    assert restored.active_safety_codes_count == 0
    ...
    assert restored.active_annealing_codes_count == 0
    assert restored.active_crystal_codes_count == 0
...
def test_institution_created_and_observed_round_trips() -> None:
    ...
    observed = InstitutionsObserved(
        ...
        active_apothecary_count=1,
        active_collegium_count=1,
        active_workshop_count=1,
        active_mason_count=1,
        active_architect_count=1,
        active_caravan_count=1,
        active_merchant_count=1,
        active_cartographer_count=1,
        active_granary_count=1,
        active_husbandman_count=1,
        active_agronomist_count=1,
        active_weaver_count=1,
        active_dyer_count=1,
        active_tailor_count=1,
        active_miner_count=1,
        active_smelter_count=1,
        active_smith_count=1,
        active_woodcutter_count=1,
        active_joiner_count=1,
    ...
    assert restored.active_apothecary_count == 1
    assert restored.active_collegium_count == 1
    assert restored.active_workshop_count == 1
    assert restored.active_mason_count == 1
    assert restored.active_architect_count == 1
    assert restored.active_caravan_count == 1
    assert restored.active_merchant_count == 1
    ...
    assert restored.active_weaver_count == 1
    assert restored.active_dyer_count == 1
    assert restored.active_tailor_count == 1
    assert restored.active_miner_count == 1
    assert restored.active_smelter_count == 1
    assert restored.active_smith_count == 1
    assert restored.active_woodcutter_count == 1
    assert restored.active_joiner_count == 1
...
def test_city_created_and_observed_round_trips() -> None:
    ...
    observed = CitiesObserved(
        ...
        active_infirmary_count=1,
        active_lazaretto_count=1,
        active_foundry_count=1,
        active_quarry_count=1,
        active_harbor_count=1,
        active_entrepot_count=1,
        active_farmstead_count=1,
        active_pastoral_count=1,
        active_mill_town_count=1,
        active_emporium_count=1,
        active_mining_camp_count=1,
        active_ironworks_count=1,
        active_timber_town_count=1,
        active_guildhall_count=1,
        active_pottery_town_count=1,
        active_kiln_quarter_count=1,
        active_glassworks_count=1,
        active_crystal_quarter_count=1,
    )
    restored = event_from_record(observed.to_record())
    ...
    assert restored.active_infirmary_count == 1
    assert restored.active_lazaretto_count == 1
    assert restored.active_foundry_count == 1
    assert restored.active_quarry_count == 1
    assert restored.active_harbor_count == 1
    ...
    assert restored.active_emporium_count == 1
    assert restored.active_mining_camp_count == 1
    assert restored.active_ironworks_count == 1
    assert restored.active_timber_town_count == 1
    assert restored.active_guildhall_count == 1
    assert restored.active_pottery_town_count == 1
    assert restored.active_kiln_quarter_count == 1
    assert restored.active_glassworks_count == 1
    assert restored.active_crystal_quarter_count == 1
    ...
    assert legacy.active_infirmary_count == 0
    assert legacy.active_lazaretto_count == 0
    assert legacy.active_foundry_count == 0
    assert legacy.active_quarry_count == 0
    assert legacy.active_harbor_count == 0
    ...
    assert legacy.active_emporium_count == 0
    assert legacy.active_mining_camp_count == 0
    assert legacy.active_ironworks_count == 0
    assert legacy.active_timber_town_count == 0
    assert legacy.active_guildhall_count == 0
    assert legacy.active_pottery_town_count == 0
    assert legacy.active_kiln_quarter_count == 0
    assert legacy.active_glassworks_count == 0
    assert legacy.active_crystal_quarter_count == 0
...
def test_infrastructure_created_and_observed_round_trips() -> None:
    ...
    observed = InfrastructuresObserved(
        ...
        active_bathhouse_count=1,
        active_bridge_count=1,
        active_scaffold_count=1,
        active_waystation_count=1,
        active_beacon_count=1,
        active_ditch_count=1,
        active_terrace_count=1,
        active_fulling_mill_count=1,
        active_warehouse_count=1,
        active_mineshaft_count=1,
        active_forge_works_count=1,
        active_lumber_yard_count=1,
        active_sawpit_count=1,
        active_kiln_yard_count=1,
        active_clay_pit_count=1,
        active_glasshouse_count=1,
        active_lehr_count=1,
    )
    restored = event_from_record(observed.to_record())
    ...
    assert restored.active_clinic_count == 1
    assert restored.active_bathhouse_count == 1
    assert restored.active_bridge_count == 1
    assert restored.active_scaffold_count == 1
    assert restored.active_waystation_count == 1
    assert restored.active_beacon_count == 1
    assert restored.active_ditch_count == 1
    assert restored.active_terrace_count == 1
    assert restored.active_fulling_mill_count == 1
    assert restored.active_warehouse_count == 1
    assert restored.active_mineshaft_count == 1
    assert restored.active_forge_works_count == 1
    assert restored.active_lumber_yard_count == 1
    assert restored.active_sawpit_count == 1
    assert restored.active_kiln_yard_count == 1
    assert restored.active_clay_pit_count == 1
    assert restored.active_glasshouse_count == 1
    assert restored.active_lehr_count == 1
...
def test_technology_created_and_observed_round_trips() -> None:
    ...
    assert restored.discovered_carpentry_count == 0
    ...
    assert restored.discovered_crystal_count == 0
...
def test_innovation_events_round_trip() -> None:
    ...
    assert restored_observed.active_sawmill_count == 0
    ...
    assert restored_observed.active_facet_count == 0
...
def test_knowledge_events_round_trip() -> None:
    ...
    assert restored_observed.carpentry_knower_count == 0
    ...
    assert restored_observed.crystal_knower_count == 0
