...
def test_default_technologies_seed_fire_through_crystal() -> None:
    """Canonical catalog has fire through crystal progression."""
    ...
    assert CAMP_JOINERY.kind is TechnologyKind.JOINERY
    assert CAMP_JOINERY.discovered is False
    assert CAMP_JOINERY.prerequisite_ids == (CAMP_CARPENTRY.technology_id,)
    assert CAMP_CABINETRY.kind is TechnologyKind.CABINETRY
    assert CAMP_CABINETRY.discovered is False
    assert CAMP_CABINETRY.prerequisite_ids == (CAMP_JOINERY.technology_id,)
    assert CAMP_CERAMICS.kind is TechnologyKind.CERAMICS
    assert CAMP_CERAMICS.discovered is False
    assert CAMP_CERAMICS.prerequisite_ids == (CAMP_CABINETRY.technology_id,)
    assert CAMP_GLAZING.kind is TechnologyKind.GLAZING
    assert CAMP_GLAZING.discovered is False
    assert CAMP_GLAZING.prerequisite_ids == (CAMP_CERAMICS.technology_id,)
    assert CAMP_PORCELAIN.kind is TechnologyKind.PORCELAIN
    assert CAMP_PORCELAIN.discovered is False
    assert CAMP_PORCELAIN.prerequisite_ids == (CAMP_GLAZING.technology_id,)
    assert CAMP_GLASSMAKING.kind is TechnologyKind.GLASSMAKING
    assert CAMP_GLASSMAKING.discovered is False
    assert CAMP_GLASSMAKING.prerequisite_ids == (CAMP_PORCELAIN.technology_id,)
    assert CAMP_OPTICS.kind is TechnologyKind.OPTICS
    assert CAMP_OPTICS.discovered is False
    assert CAMP_OPTICS.prerequisite_ids == (CAMP_GLASSMAKING.technology_id,)
    assert CAMP_CRYSTAL.kind is TechnologyKind.CRYSTAL
    assert CAMP_CRYSTAL.discovered is False
    assert CAMP_CRYSTAL.prerequisite_ids == (CAMP_OPTICS.technology_id,)
...
def test_census_technologies_counts() -> None:
    ...
    assert snap.technology_count == 37
    assert snap.discovered_count == 1
    assert snap.undiscovered_count == 36
    ...
    assert snap.discovered_joinery_count == 0
    assert snap.discovered_cabinetry_count == 0
    assert snap.discovered_ceramics_count == 0
    assert snap.discovered_glazing_count == 0
    assert snap.discovered_porcelain_count == 0
    assert snap.discovered_glassmaking_count == 0
    assert snap.discovered_optics_count == 0
    assert snap.discovered_crystal_count == 0
    assert snap.locked_count == 35
...
def test_joinery_locked_until_carpentry_discovered() -> None:
    """Joinery cannot be discovered until carpentry is already known."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        technologies=default_technologies(),
    )
    assert prerequisites_met(world, CAMP_JOINERY) is False
    assert discover_technology(world, CAMP_JOINERY.technology_id) is None
    ...
    for technology in (
        ...
        assert updated is not None
        current = updated
    assert prerequisites_met(current, CAMP_JOINERY) is False
    assert discover_technology(current, CAMP_JOINERY.technology_id) is None
...
def test_cabinetry_locked_until_joinery_discovered() -> None:
    """Cabinetry cannot be discovered until joinery is already known."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        technologies=default_technologies(),
    )
    assert prerequisites_met(world, CAMP_CABINETRY) is False
    assert discover_technology(world, CAMP_CABINETRY.technology_id) is None
    ...
    for technology in (
        ...
        assert updated is not None
        current = updated
    assert prerequisites_met(current, CAMP_CABINETRY) is False
    assert discover_technology(current, CAMP_CABINETRY.technology_id) is None
...
def test_ceramics_locked_until_cabinetry_discovered() -> None:
    """Ceramics cannot be discovered until cabinetry is already known."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        technologies=default_technologies(),
    )
    assert prerequisites_met(world, CAMP_CERAMICS) is False
    assert discover_technology(world, CAMP_CERAMICS.technology_id) is None
    ...
    for technology in (
        ...
        assert updated is not None
        current = updated
    assert prerequisites_met(current, CAMP_CERAMICS) is False
    assert discover_technology(current, CAMP_CERAMICS.technology_id) is None
...
def test_glazing_locked_until_ceramics_discovered() -> None:
    """Glazing cannot be discovered until ceramics is already known."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        technologies=default_technologies(),
    )
    assert prerequisites_met(world, CAMP_GLAZING) is False
    assert discover_technology(world, CAMP_GLAZING.technology_id) is None
    ...
    for technology in (
        ...
        assert updated is not None
        current = updated
    assert prerequisites_met(current, CAMP_GLAZING) is False
    assert discover_technology(current, CAMP_GLAZING.technology_id) is None
...
def test_porcelain_locked_until_glazing_discovered() -> None:
    """Porcelain cannot be discovered until glazing is already known."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        technologies=default_technologies(),
    )
    assert prerequisites_met(world, CAMP_PORCELAIN) is False
    assert discover_technology(world, CAMP_PORCELAIN.technology_id) is None
    ...
    for technology in (
        ...
        assert updated is not None
        current = updated
    assert prerequisites_met(current, CAMP_PORCELAIN) is False
    assert discover_technology(current, CAMP_PORCELAIN.technology_id) is None
...
def test_glassmaking_locked_until_porcelain_discovered() -> None:
    """Glassmaking cannot be discovered until porcelain is already known."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        technologies=default_technologies(),
    )
    assert prerequisites_met(world, CAMP_GLASSMAKING) is False
    assert discover_technology(world, CAMP_GLASSMAKING.technology_id) is None
    ...
    for technology in (
        ...
        assert updated is not None
        current = updated
    assert prerequisites_met(current, CAMP_GLASSMAKING) is False
    assert discover_technology(current, CAMP_GLASSMAKING.technology_id) is None

    with_porcelain = discover_technology(current, CAMP_PORCELAIN.technology_id)
    assert with_porcelain is not None
    assert prerequisites_met(with_porcelain, CAMP_GLASSMAKING) is True
    with_glassmaking = discover_technology(
        with_porcelain, CAMP_GLASSMAKING.technology_id
    )
    assert with_glassmaking is not None
    assert with_glassmaking.technologies[34].discovered is True


def test_optics_locked_until_glassmaking_discovered() -> None:
    """Optics cannot be discovered until glassmaking is already known."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        technologies=default_technologies(),
    )
    assert prerequisites_met(world, CAMP_OPTICS) is False
    assert discover_technology(world, CAMP_OPTICS.technology_id) is None
    ...
    for technology in (
        ...
        assert updated is not None
        current = updated
    assert prerequisites_met(current, CAMP_OPTICS) is False
    assert discover_technology(current, CAMP_OPTICS.technology_id) is None
...
def test_crystal_locked_until_optics_discovered() -> None:
    """Crystal cannot be discovered until optics is already known."""
    world = _world(
        Agent.create(agent_id=0, name="A"),
        technologies=default_technologies(),
    )
    ...
    for technology in (
        ...
        assert updated is not None
        current = updated
    assert prerequisites_met(current, CAMP_CRYSTAL) is False
    assert discover_technology(current, CAMP_CRYSTAL.technology_id) is None
