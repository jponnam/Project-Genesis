...
from civitas.domain import (
    ...
    OPTICIAN_TEACHINGS_PER_KNOWER_BONUS,
    OPTICS_PRODUCE_ENERGY_DISCOUNT,
...
def test_forest_management_boosts_wood_gather_for_living_subject() -> None:
    """Active FOREST_MANAGEMENT adds a subject-scoped wood gather bonus."""
    ...
    world = World(
        ...
        locations=(CAMP_LOCATION,),
        governments=(Government.create(0, "Camp", 0, (0,)),),
        laws=(forest_management,),
        agents=(Agent.create(agent_id=0, name="A"),),
    )
    agent = world.agents[0]
    assert forest_management_wood_bonus_for(world, agent) == (
        FOREST_MANAGEMENT_WOOD_GATHER_BONUS
    )
    # gather_amount_bonus stays location-scoped, unaffected by forest management.
    assert gather_amount_bonus(world, "wood", location_id=agent.location_id) == 0
    assert effective_gather_amount(world, "wood", agent=agent) == (
        DEFAULT_GATHER_AMOUNT + FOREST_MANAGEMENT_WOOD_GATHER_BONUS
    )
    # Without an agent there is no subject to scope the bonus to.
    assert effective_gather_amount(world, "wood") == DEFAULT_GATHER_AMOUNT


def test_forest_management_stacks_with_wood_bonuses() -> None:
    """Forest management stacks with sawmill, coppice, scaffold, and law bonuses."""
    discovered_forestry = CAMP_FORESTRY.model_copy(update={"discovered": True})
    discovered_carpentry = CAMP_CARPENTRY.model_copy(update={"discovered": True})
    active_coppice = CAMP_COPPICE.model_copy(update={"active": True})
    active_sawmill = CAMP_SAWMILL.model_copy(update={"active": True})
    ...
    timber_rights = Law.create(1, 0, "Camp Timber Rights", LawKind.TIMBER_RIGHTS)
    forest_management = Law.create(
        2, 0, "Camp Forest Management", LawKind.FOREST_MANAGEMENT
    )
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
...
def test_woodcutter_boosts_wood_gather_for_colocated_agents() -> None:
    """Active woodcutter seats add a wood gather bonus at their location."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        governments=(Government.create(0, "Camp", 0, (0,)),),
        institutions=(
            Institution.create(0, 0, 0, "Camp Woodcutter", InstitutionKind.WOODCUTTER),
        ),
        agents=(Agent.create(agent_id=0, name="A"),),
    )
    agent = world.agents[0]
    assert location_has_active_woodcutter(world, agent.location_id) is True
    assert gather_amount_bonus(world, "wood", location_id=agent.location_id) == (
        WOODCUTTER_WOOD_GATHER_BONUS
    )
    assert gather_amount_bonus(world, "wood") == 0
    assert effective_gather_amount(world, "wood", agent=agent) == (
        DEFAULT_GATHER_AMOUNT + WOODCUTTER_WOOD_GATHER_BONUS
    )


def test_woodcutter_stacks_with_sawmill_coppice_scaffold_timber_and_pastoral() -> None:
    """Woodcutter seat bonus stacks with sawmill, coppice, scaffold, pastoral."""
    discovered_forestry = CAMP_FORESTRY.model_copy(update={"discovered": True})
    discovered_carpentry = CAMP_CARPENTRY.model_copy(update={"discovered": True})
    active_coppice = CAMP_COPPICE.model_copy(update={"active": True})
    active_sawmill = CAMP_SAWMILL.model_copy(update={"active": True})
...
def test_lumber_yard_boosts_wood_gather_for_colocated_agents() -> None:
    """Active lumber yards add a wood gather bonus at their seat location."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        governments=(Government.create(0, "Camp", 0, (0,)),),
    ...
    agent = world.agents[0]
    assert location_has_active_lumber_yard(world, agent.location_id) is True
    assert gather_amount_bonus(world, "wood", location_id=agent.location_id) == (
        LUMBER_YARD_WOOD_GATHER_BONUS
    )
    assert gather_amount_bonus(world, "wood") == 0
    assert effective_gather_amount(world, "wood", agent=agent) == (
        DEFAULT_GATHER_AMOUNT + LUMBER_YARD_WOOD_GATHER_BONUS
    )


def test_lumber_yard_stacks_with_sawmill_coppice_scaffold_woodcutter_pastoral() -> None:
    """Lumber yard seat bonus stacks with every other wood gather source."""
    discovered_forestry = CAMP_FORESTRY.model_copy(update={"discovered": True})
    discovered_carpentry = CAMP_CARPENTRY.model_copy(update={"discovered": True})
    active_coppice = CAMP_COPPICE.model_copy(update={"active": True})
    active_sawmill = CAMP_SAWMILL.model_copy(update={"active": True})
    ...
    world = World(
        ...
        infrastructure=(
            ...
            Infrastructure.create(
                1, 0, 1, 1, "Pastoral Lumber Yard", InfrastructureKind.LUMBER_YARD
            ),
        ),
        institutions=(
            Institution.create(0, 0, 1, "Camp Woodcutter", InstitutionKind.WOODCUTTER),
...
def test_collegium_stacks_with_all_teaching_bonuses() -> None:
    ...
    world = World(
        ...
        institutions=(
            ...
            Institution.create(9, 0, 1, "Forum Tilewright", InstitutionKind.TILEWRIGHT),
            Institution.create(10, 0, 1, "Forum Optician", InstitutionKind.OPTICIAN),
        ),
        agents=(Agent.create(agent_id=0, name="A", location_id=1),),
    )
    agent = world.agents[0]
    ...
    assert location_has_active_smith(world, agent.location_id) is True
    assert location_has_active_carver(world, agent.location_id) is True
    assert location_has_active_tilewright(world, agent.location_id) is True
    assert location_has_active_optician(world, agent.location_id) is True
    assert (
        effective_teachings_per_knower(
            world, base=DEFAULT_TEACHINGS_PER_KNOWER, agent=agent
        )
...
def test_smith_boosts_teachings_and_stacks_with_tailor() -> None:
    """Active smith seat bonus stacks with tailor and prior sources."""
    discovered_pottery = CAMP_POTTERY.model_copy(update={"discovered": True})
    discovered_irrigation = CAMP_IRRIGATION.model_copy(update={"discovered": True})
    discovered_metallurgy = CAMP_METALLURGY.model_copy(update={"discovered": True})
    discovered_writing = CAMP_WRITING.model_copy(update={"discovered": True})
    ...
    # Without agent/location, smith (seat-scoped) and curriculum do not apply.
    assert (
        effective_teachings_per_knower(world, base=DEFAULT_TEACHINGS_PER_KNOWER)
        == DEFAULT_TEACHINGS_PER_KNOWER + WRITING_TEACHINGS_PER_KNOWER_BONUS
    )
    smith_only = World(
        ...
        governments=(Government.create(0, "Camp", 0, (0,)),),
        institutions=(
            Institution.create(0, 0, 0, "Camp Smith", InstitutionKind.SMITH),
        ),
        agents=(Agent.create(agent_id=0, name="A"),),
    )
    assert location_has_active_smith(smith_only, 0) is True
    assert (
        effective_teachings_per_knower(
            smith_only,
            base=DEFAULT_TEACHINGS_PER_KNOWER,
            agent=smith_only.agents[0],
        )
        == DEFAULT_TEACHINGS_PER_KNOWER + SMITH_TEACHINGS_PER_KNOWER_BONUS
    )
    bare = _world()
    assert location_has_active_smith(bare, bare.agents[0].location_id) is False


def test_carver_boosts_teachings_and_stacks_with_smith() -> None:
    """Active carver seat bonus stacks with smith and prior sources."""
    discovered_pottery = CAMP_POTTERY.model_copy(update={"discovered": True})
    discovered_irrigation = CAMP_IRRIGATION.model_copy(update={"discovered": True})
    discovered_metallurgy = CAMP_METALLURGY.model_copy(update={"discovered": True})
    discovered_writing = CAMP_WRITING.model_copy(update={"discovered": True})
    ...
    # Without agent/location, carver (seat-scoped) and curriculum do not apply.
    assert (
        effective_teachings_per_knower(world, base=DEFAULT_TEACHINGS_PER_KNOWER)
        == DEFAULT_TEACHINGS_PER_KNOWER + WRITING_TEACHINGS_PER_KNOWER_BONUS
    )
    carver_only = World(
        ...
        governments=(Government.create(0, "Camp", 0, (0,)),),
        institutions=(
            Institution.create(0, 0, 0, "Camp Carver", InstitutionKind.CARVER),
        ),
        agents=(Agent.create(agent_id=0, name="A"),),
    )
    assert location_has_active_carver(carver_only, 0) is True
    assert (
        effective_teachings_per_knower(
            carver_only,
            base=DEFAULT_TEACHINGS_PER_KNOWER,
            agent=carver_only.agents[0],
        )
        == DEFAULT_TEACHINGS_PER_KNOWER + CARVER_TEACHINGS_PER_KNOWER_BONUS
    )
    bare = _world()
...
def test_tilewright_boosts_teachings_and_stacks_with_carver() -> None:
    ...
    world = World(
        ...
        institutions=(
            ...
            Institution.create(7, 0, 0, "Camp Smith", InstitutionKind.SMITH),
            Institution.create(8, 0, 0, "Camp Carver", InstitutionKind.CARVER),
            Institution.create(9, 0, 0, "Camp Tilewright", InstitutionKind.TILEWRIGHT),
        ),
        agents=(Agent.create(agent_id=0, name="A"),),
    )
    agent = world.agents[0]
    ...
    # Without agent/location, tilewright (seat-scoped) and curriculum do not apply.
    assert (
        effective_teachings_per_knower(world, base=DEFAULT_TEACHINGS_PER_KNOWER)
        == DEFAULT_TEACHINGS_PER_KNOWER + WRITING_TEACHINGS_PER_KNOWER_BONUS
    )
    tilewright_only = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        governments=(Government.create(0, "Camp", 0, (0,)),),
        institutions=(
            Institution.create(0, 0, 0, "Camp Tilewright", InstitutionKind.TILEWRIGHT),
        ),
        agents=(Agent.create(agent_id=0, name="A"),),
    )
    assert location_has_active_tilewright(tilewright_only, 0) is True
    assert (
        effective_teachings_per_knower(
            tilewright_only,
            base=DEFAULT_TEACHINGS_PER_KNOWER,
            agent=tilewright_only.agents[0],
        )
        == DEFAULT_TEACHINGS_PER_KNOWER + TILEWRIGHT_TEACHINGS_PER_KNOWER_BONUS
    )
    bare = _world()
...
def test_optician_boosts_teachings_and_stacks_with_tilewright() -> None:
    ...
    world = World(
        ...
        institutions=(
            ...
            Institution.create(8, 0, 0, "Camp Carver", InstitutionKind.CARVER),
            Institution.create(9, 0, 0, "Camp Tilewright", InstitutionKind.TILEWRIGHT),
            Institution.create(10, 0, 0, "Camp Optician", InstitutionKind.OPTICIAN),
        ),
        agents=(Agent.create(agent_id=0, name="A"),),
    )
    agent = world.agents[0]
    ...
    # Without agent/location, optician (seat-scoped) and curriculum do not apply.
    assert (
        effective_teachings_per_knower(world, base=DEFAULT_TEACHINGS_PER_KNOWER)
        == DEFAULT_TEACHINGS_PER_KNOWER + WRITING_TEACHINGS_PER_KNOWER_BONUS
    )
    optician_only = World(
        ...
        governments=(Government.create(0, "Camp", 0, (0,)),),
        institutions=(
            Institution.create(0, 0, 0, "Camp Optician", InstitutionKind.OPTICIAN),
        ),
        agents=(Agent.create(agent_id=0, name="A"),),
    )
    assert location_has_active_optician(optician_only, 0) is True
    assert (
        effective_teachings_per_knower(
            optician_only,
            base=DEFAULT_TEACHINGS_PER_KNOWER,
            agent=optician_only.agents[0],
        )
        == DEFAULT_TEACHINGS_PER_KNOWER + OPTICIAN_TEACHINGS_PER_KNOWER_BONUS
    )
    bare = _world()
    assert location_has_active_optician(bare, bare.agents[0].location_id) is False
...
def test_firing_codes_reduces_produce_energy_and_stacks_with_guild() -> None:
    ...
    agent = world.agents[0]
    expected_discount = (
        GUILD_PRODUCE_ENERGY_DISCOUNT + FIRING_CODES_PRODUCE_ENERGY_DISCOUNT
    )
    assert firing_codes_produce_discount_for(world, agent) == (
        FIRING_CODES_PRODUCE_ENERGY_DISCOUNT
    )
    assert produce_energy_discount(world, agent) == pytest.approx(expected_discount)
    assert effective_produce_energy_cost(
    ...
    assert effective_produce_energy_cost(
        ...
        DEFAULT_PRODUCE_ENERGY_COST - FIRING_CODES_PRODUCE_ENERGY_DISCOUNT
    )
    # census_effects produce path omits statute discounts (firing codes).
    assert census_effects(world).produce_energy_cost_bps == round(
        (DEFAULT_PRODUCE_ENERGY_COST - GUILD_PRODUCE_ENERGY_DISCOUNT) * 10_000
    )


def test_clay_codes_reduces_produce_energy_and_stacks_with_guild() -> None:
    """Clay-code laws discount PRODUCE energy and stack with guild seats."""
    law = Law.create(0, 0, "Camp Clay Codes", LawKind.CLAY_CODES)
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        governments=(Government.create(0, "Camp", 0, (0,)),),
    ...
    agent = world.agents[0]
    expected_discount = (
        GUILD_PRODUCE_ENERGY_DISCOUNT + CLAY_CODES_PRODUCE_ENERGY_DISCOUNT
    )
    assert clay_codes_produce_discount_for(world, agent) == (
        CLAY_CODES_PRODUCE_ENERGY_DISCOUNT
    )
    assert produce_energy_discount(world, agent) == pytest.approx(expected_discount)
    assert effective_produce_energy_cost(
...
def test_bellows_raises_produce_discount_society_wide() -> None:
    """Active bellows discounts PRODUCE energy for every agent society-wide."""
    discovered_smithing = CAMP_SMITHING.model_copy(update={"discovered": True})
    active_bellows = CAMP_BELLOWS.model_copy(update={"active": True})
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        technologies=tuple(
            discovered_smithing
            if item.technology_id == CAMP_SMITHING.technology_id
            else item
            for item in default_technologies()
        ),
        innovations=tuple(
    ...
    agent = world.agents[0]
    expected = DEFAULT_PRODUCE_ENERGY_COST - SMITHING_PRODUCE_ENERGY_DISCOUNT
    assert effective_produce_energy_cost(
        world,
        agent,
        base=DEFAULT_PRODUCE_ENERGY_COST,
...
def test_lathe_raises_produce_discount_society_wide() -> None:
    """Active lathe discounts PRODUCE energy for every agent society-wide."""
    discovered_toolmaking = CAMP_TOOLMAKING.model_copy(update={"discovered": True})
    active_lathe = CAMP_LATHE.model_copy(update={"active": True})
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        technologies=tuple(
            discovered_toolmaking
            if item.technology_id == CAMP_TOOLMAKING.technology_id
            else item
            for item in default_technologies()
        ),
        innovations=tuple(
    ...
    agent = world.agents[0]
    expected = DEFAULT_PRODUCE_ENERGY_COST - TOOLMAKING_PRODUCE_ENERGY_DISCOUNT
    assert effective_produce_energy_cost(
        world,
        agent,
        base=DEFAULT_PRODUCE_ENERGY_COST,
...
def test_plane_reduces_produce_energy_and_stacks_with_guild_abacus() -> None:
    ...
    active_abacus = CAMP_ABACUS.model_copy(update={"active": True})
    active_lathe = CAMP_LATHE.model_copy(update={"active": True})
    active_plane = CAMP_PLANE.model_copy(update={"active": True})
    innovations = tuple(
        active_abacus
        if item.innovation_id == CAMP_ABACUS.innovation_id
        else active_lathe
...
def test_plane_raises_produce_discount_society_wide() -> None:
    """Active plane discounts PRODUCE energy for every agent society-wide."""
    discovered_joinery = CAMP_JOINERY.model_copy(update={"discovered": True})
    active_plane = CAMP_PLANE.model_copy(update={"active": True})
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        technologies=tuple(
            discovered_joinery
            if item.technology_id == CAMP_JOINERY.technology_id
            else item
            for item in default_technologies()
        ),
        innovations=tuple(
    ...
    agent = world.agents[0]
    expected = DEFAULT_PRODUCE_ENERGY_COST - JOINERY_PRODUCE_ENERGY_DISCOUNT
    assert effective_produce_energy_cost(
        world,
        agent,
        base=DEFAULT_PRODUCE_ENERGY_COST,
...
def test_dovetail_reduces_produce_energy_and_stacks_with_guild_abacus() -> None:
    ...
    active_lathe = CAMP_LATHE.model_copy(update={"active": True})
    active_plane = CAMP_PLANE.model_copy(update={"active": True})
    active_dovetail = CAMP_DOVETAIL.model_copy(update={"active": True})
    innovations = tuple(
        active_abacus
        if item.innovation_id == CAMP_ABACUS.innovation_id
        else active_lathe
        ...
        if item.innovation_id == CAMP_PLANE.innovation_id
        else active_dovetail
        if item.innovation_id == CAMP_DOVETAIL.innovation_id
        else item
        for item in default_innovations()
    )
    world = World(
...
def test_dovetail_raises_produce_discount_society_wide() -> None:
    """Active dovetail discounts PRODUCE energy for every agent society-wide."""
    discovered_cabinetry = CAMP_CABINETRY.model_copy(update={"discovered": True})
    active_dovetail = CAMP_DOVETAIL.model_copy(update={"active": True})
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        technologies=tuple(
            discovered_cabinetry
            if item.technology_id == CAMP_CABINETRY.technology_id
            else item
            for item in default_technologies()
        ),
        innovations=tuple(
...
def test_kaolin_raises_produce_discount_society_wide() -> None:
    """Active kaolin discounts PRODUCE energy for every agent society-wide."""
    discovered_porcelain = CAMP_PORCELAIN.model_copy(update={"discovered": True})
    active_kaolin = CAMP_KAOLIN.model_copy(update={"active": True})
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        technologies=tuple(
            discovered_porcelain
            if item.technology_id == CAMP_PORCELAIN.technology_id
            else item
            for item in default_technologies()
        ),
        innovations=tuple(
...
def test_lens_raises_produce_discount_society_wide() -> None:
    """Active lens discounts PRODUCE energy for every agent society-wide."""
    discovered_optics = CAMP_OPTICS.model_copy(update={"discovered": True})
    active_lens = CAMP_LENS.model_copy(update={"active": True})
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        technologies=tuple(
            discovered_optics
            if item.technology_id == CAMP_OPTICS.technology_id
            else item
            for item in default_technologies()
        ),
        innovations=tuple(
...
def test_smelter_reduces_produce_energy_for_colocated_agents() -> None:
    """Active smelters discount PRODUCE energy at their seat location."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        governments=(Government.create(0, "Camp", 0, (0,)),),
        institutions=(
            Institution.create(0, 0, 0, "Camp Smelter", InstitutionKind.SMELTER),
        ),
        agents=(Agent.create(agent_id=0, name="A"),),
    )
    agent = world.agents[0]
    ...
    assert effective_produce_energy_cost(
        ...
        agent,
        base=DEFAULT_PRODUCE_ENERGY_COST,
    ) == pytest.approx(DEFAULT_PRODUCE_ENERGY_COST - SMELTER_PRODUCE_ENERGY_DISCOUNT)
    assert census_effects(world).produce_energy_cost_bps == round(
        (DEFAULT_PRODUCE_ENERGY_COST - SMELTER_PRODUCE_ENERGY_DISCOUNT) * 10_000
    )
    bare = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
    ...
    assert location_has_active_smelter(bare, bare.agents[0].location_id) is False


def test_smelter_stacks_with_guild_workshop_abacus_and_pulley() -> None:
    """Smelter seat discount stacks with guild, workshop, abacus, and pulley."""
    discovered_pottery = CAMP_POTTERY.model_copy(update={"discovered": True})
    discovered_irrigation = CAMP_IRRIGATION.model_copy(update={"discovered": True})
    discovered_metallurgy = CAMP_METALLURGY.model_copy(update={"discovered": True})
    discovered_writing = CAMP_WRITING.model_copy(update={"discovered": True})
    ...
    world = World(
        ...
        institutions=(
            Institution.create(0, 0, 0, "Camp Guild", InstitutionKind.GUILD),
            Institution.create(1, 0, 0, "Camp Workshop", InstitutionKind.WORKSHOP),
            Institution.create(2, 0, 0, "Camp Smelter", InstitutionKind.SMELTER),
        ),
        technologies=(
            CAMP_FIRE,
            discovered_pottery,
...
def test_glassblower_reduces_produce_energy_for_colocated_agents() -> None:
    """Active glassblowers discount PRODUCE energy at their seat location."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        governments=(Government.create(0, "Camp", 0, (0,)),),
    ...
    agent = world.agents[0]
    assert location_has_active_glassblower(world, agent.location_id) is True
    assert effective_produce_energy_cost(
        world,
        agent,
        base=DEFAULT_PRODUCE_ENERGY_COST,
    ) == pytest.approx(
        DEFAULT_PRODUCE_ENERGY_COST - GLASSBLOWER_PRODUCE_ENERGY_DISCOUNT
    )
    assert census_effects(world).produce_energy_cost_bps == round(
        (DEFAULT_PRODUCE_ENERGY_COST - GLASSBLOWER_PRODUCE_ENERGY_DISCOUNT) * 10_000
    )
    bare = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
...
def test_forge_works_reduces_produce_energy_for_colocated_agents() -> None:
    """Active forge works discount PRODUCE energy at their seat location."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        governments=(Government.create(0, "Camp", 0, (0,)),),
    ...
    assert effective_produce_energy_cost(
        ...
        DEFAULT_PRODUCE_ENERGY_COST - FORGE_WORKS_PRODUCE_ENERGY_DISCOUNT
    )
    assert census_effects(world).produce_energy_cost_bps == round(
        (DEFAULT_PRODUCE_ENERGY_COST - FORGE_WORKS_PRODUCE_ENERGY_DISCOUNT) * 10_000
    )
    bare = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
    ...
    assert location_has_active_forge_works(bare, bare.agents[0].location_id) is False


def test_forge_works_stacks_with_guild_workshop_weaver_and_fulling_mill() -> None:
    """Forge works seat discount stacks with guild/workshop/weaver/fulling mill."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        governments=(Government.create(0, "Camp", 0, (0,)),),
        ...
        infrastructure=(
            ...
            Infrastructure.create(
                1, 0, 0, 0, "Camp Forge Works", InfrastructureKind.FORGE_WORKS
            ),
        ),
        agents=(Agent.create(agent_id=0, name="A"),),
    )
