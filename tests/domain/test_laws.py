...
def test_default_laws_seed_camp_poll_tax() -> None:
    ...
    assert all(law.kind is not LawKind.CRYSTAL_CODES for law in default_laws())
...
def test_world_rejects_duplicate_active_firing_codes() -> None:
    """At most one active FIRING_CODES law per government."""
...
def test_world_rejects_duplicate_active_crystal_codes() -> None:
    """At most one active CRYSTAL_CODES law per government."""
    left = Law.create(0, 0, "Crystal Codes A", LawKind.CRYSTAL_CODES)
    right = Law.create(1, 0, "Crystal Codes B", LawKind.CRYSTAL_CODES)
    with pytest.raises(ValidationError, match="CRYSTAL_CODES"):
        _world(Agent.create(agent_id=0, name="A"), laws=(left, right))


def test_world_rejects_duplicate_active_safety_codes() -> None:
    """At most one active SAFETY_CODES law per government."""
    left = Law.create(0, 0, "Safety Codes A", LawKind.SAFETY_CODES)
    right = Law.create(1, 0, "Safety Codes B", LawKind.SAFETY_CODES)
    with pytest.raises(ValidationError, match="SAFETY_CODES"):
        _world(Agent.create(agent_id=0, name="A"), laws=(left, right))
...
def test_enact_timber_rights_and_uniqueness() -> None:
    """TIMBER_RIGHTS enacts once per government; kind alone enables wood bonus."""
    world = _world(Agent.create(agent_id=0, name="A"))
    timber_rights = Law.create(0, 0, "Camp Timber Rights", LawKind.TIMBER_RIGHTS)
    enacted = enact_law(world, timber_rights)
    assert enacted is not None
    assert active_timber_rights_law(enacted, 0) == timber_rights
    assert timber_rights_wood_bonus_for(enacted, enacted.agents[0]) == (
        TIMBER_RIGHTS_WOOD_GATHER_BONUS
    )
    assert TIMBER_RIGHTS_WOOD_GATHER_BONUS == 1
    duplicate = Law.create(1, 0, "Other Timber Rights", LawKind.TIMBER_RIGHTS)
    assert enact_law(enacted, duplicate) is None
    # Other unique kinds may coexist with TIMBER_RIGHTS.
    conservation = Law.create(1, 0, "Camp Conservation", LawKind.CONSERVATION)
    with_conservation = enact_law(enacted, conservation)
    assert with_conservation is not None
    assert active_timber_rights_law(with_conservation, 0) == timber_rights


def test_timber_rights_bonus_requires_living_subject() -> None:
    """Only living agents under TIMBER_RIGHTS receive the wood gather bonus."""
    timber_rights = Law.create(0, 0, "Camp Timber Rights", LawKind.TIMBER_RIGHTS)
    world = _world(Agent.create(agent_id=0, name="A"), laws=(timber_rights,))
    assert timber_rights_wood_bonus_for(world, world.agents[0]) == (
        TIMBER_RIGHTS_WOOD_GATHER_BONUS
    )
    dead = world.agents[0].model_copy(
        update={
    ...
    assert timber_rights_wood_bonus_for(world, dead) == 0
    ungoverned = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        governments=(),
    ...
    assert timber_rights_wood_bonus_for(ungoverned, ungoverned.agents[0]) == 0
    bare = _world(Agent.create(agent_id=0, name="A"))
...
def test_enact_firing_codes_and_uniqueness() -> None:
    """FIRING_CODES enacts once per government; kind alone enables discount."""
    world = _world(Agent.create(agent_id=0, name="A"))
    firing_codes = Law.create(0, 0, "Camp Firing Codes", LawKind.FIRING_CODES)
    enacted = enact_law(world, firing_codes)
    assert enacted is not None
    assert active_firing_codes_law(enacted, 0) == firing_codes
    assert firing_codes_produce_discount_for(enacted, enacted.agents[0]) == (
        FIRING_CODES_PRODUCE_ENERGY_DISCOUNT
    )
    assert FIRING_CODES_PRODUCE_ENERGY_DISCOUNT == 0.02
    duplicate = Law.create(1, 0, "Other Firing Codes", LawKind.FIRING_CODES)
    assert enact_law(enacted, duplicate) is None
    # Other unique kinds may coexist with FIRING_CODES.
    ...
    assert active_firing_codes_law(with_safety, 0) == firing_codes
    assert (
        firing_codes_produce_discount_for(
            with_safety,
...
def test_firing_codes_discount_requires_living_subject() -> None:
    """Only living agents under FIRING_CODES receive the produce discount."""
    firing_codes = Law.create(0, 0, "Camp Firing Codes", LawKind.FIRING_CODES)
    world = _world(Agent.create(agent_id=0, name="A"), laws=(firing_codes,))
    assert firing_codes_produce_discount_for(world, world.agents[0]) == (
        FIRING_CODES_PRODUCE_ENERGY_DISCOUNT
    )
    dead = world.agents[0].model_copy(
        update={
    ...
    assert firing_codes_produce_discount_for(world, dead) == 0.0
    ungoverned = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        governments=(),
...
def test_enact_crystal_codes_and_uniqueness() -> None:
    """CRYSTAL_CODES enacts once per government; kind alone enables discount."""
    world = _world(Agent.create(agent_id=0, name="A"))
    crystal_codes = Law.create(0, 0, "Camp Crystal Codes", LawKind.CRYSTAL_CODES)
    enacted = enact_law(world, crystal_codes)
    assert enacted is not None
    assert active_crystal_codes_law(enacted, 0) == crystal_codes
    assert crystal_codes_produce_discount_for(enacted, enacted.agents[0]) == (
        CRYSTAL_CODES_PRODUCE_ENERGY_DISCOUNT
    )
    assert CRYSTAL_CODES_PRODUCE_ENERGY_DISCOUNT == 0.02
    duplicate = Law.create(1, 0, "Other Crystal Codes", LawKind.CRYSTAL_CODES)
    assert enact_law(enacted, duplicate) is None
    # Other unique kinds may coexist with CRYSTAL_CODES.
    safety_codes = Law.create(1, 0, "Camp Safety Codes", LawKind.SAFETY_CODES)
    with_safety = enact_law(enacted, safety_codes)
    assert with_safety is not None
    assert active_crystal_codes_law(with_safety, 0) == crystal_codes
    assert (
        crystal_codes_produce_discount_for(
            with_safety,
            with_safety.agents[0],
...
def test_crystal_codes_discount_requires_living_subject() -> None:
    """Only living agents under CRYSTAL_CODES receive the produce discount."""
    crystal_codes = Law.create(0, 0, "Camp Crystal Codes", LawKind.CRYSTAL_CODES)
    world = _world(Agent.create(agent_id=0, name="A"), laws=(crystal_codes,))
    assert crystal_codes_produce_discount_for(world, world.agents[0]) == (
        CRYSTAL_CODES_PRODUCE_ENERGY_DISCOUNT
    )
    dead = world.agents[0].model_copy(
        update={
    ...
    assert crystal_codes_produce_discount_for(world, dead) == 0.0
    ungoverned = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        governments=(),
...
def test_enact_safety_codes_and_uniqueness() -> None:
    """SAFETY_CODES enacts once per government; kind alone enables discount."""
    world = _world(Agent.create(agent_id=0, name="A"))
    safety_codes = Law.create(0, 0, "Camp Safety Codes", LawKind.SAFETY_CODES)
    enacted = enact_law(world, safety_codes)
    assert enacted is not None
    assert active_safety_codes_law(enacted, 0) == safety_codes
    assert safety_codes_produce_discount_for(enacted, enacted.agents[0]) == (
        SAFETY_CODES_PRODUCE_ENERGY_DISCOUNT
    )
    assert SAFETY_CODES_PRODUCE_ENERGY_DISCOUNT == 0.02
    duplicate = Law.create(1, 0, "Other Safety Codes", LawKind.SAFETY_CODES)
    assert enact_law(enacted, duplicate) is None
    # Other unique kinds may coexist with SAFETY_CODES.
    labor = Law.create(1, 0, "Camp Labor", LawKind.LABOR)
    with_labor = enact_law(enacted, labor)
    assert with_labor is not None
    assert active_safety_codes_law(with_labor, 0) == safety_codes
    assert (
        safety_codes_produce_discount_for(
            with_labor,
            with_labor.agents[0],
...
def test_safety_codes_discount_requires_living_subject() -> None:
    """Only living agents under SAFETY_CODES receive the produce discount."""
    safety_codes = Law.create(0, 0, "Camp Safety Codes", LawKind.SAFETY_CODES)
    world = _world(Agent.create(agent_id=0, name="A"), laws=(safety_codes,))
    assert safety_codes_produce_discount_for(world, world.agents[0]) == (
        SAFETY_CODES_PRODUCE_ENERGY_DISCOUNT
    )
    dead = world.agents[0].model_copy(
        update={
    ...
    assert safety_codes_produce_discount_for(world, dead) == 0.0
    ungoverned = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        governments=(),
...
def test_census_laws_counts() -> None:
    ...
    safety_codes = Law.create(18, 0, "Safety Codes", LawKind.SAFETY_CODES, active=True)
    ...
    crystal_codes = Law.create(
        24, 0, "Crystal Codes", LawKind.CRYSTAL_CODES, active=True
    )
    world = _world(
        Agent.create(agent_id=0, name="A"),
        laws=(
    ...
    assert snap.active_crystal_codes_count == 1
