...
def test_bootstrap_uses_smithing_technology_fact() -> None:
    """Discovered smithing bootstraps through the generic tech fact mapping."""
    prior = Knowledge(
        facts=frozenset(
            {
                FIRE_FACT,
    ...
    world, gains = bootstrap_discovered_knowledge(with_smithing)
    assert len(gains) == 1
    assert gains[0].fact == SMITHING_FACT
    assert world.agents[0].knowledge.knows(SMITHING_FACT)


def test_bootstrap_uses_toolmaking_technology_fact() -> None:
    """Discovered toolmaking bootstraps through the generic tech fact mapping."""
    prior = Knowledge(
        facts=frozenset(
            {
                FIRE_FACT,
    ...
    world, gains = bootstrap_discovered_knowledge(with_toolmaking)
    assert len(gains) == 1
    assert gains[0].fact == TOOLMAKING_FACT
    assert world.agents[0].knowledge.knows(TOOLMAKING_FACT)


def test_bootstrap_uses_carpentry_technology_fact() -> None:
    """Discovered carpentry bootstraps through the generic tech fact mapping."""
    prior = Knowledge(
        facts=frozenset(
            {
                FIRE_FACT,
    ...
    world, gains = bootstrap_discovered_knowledge(with_carpentry)
    assert len(gains) == 1
    assert gains[0].fact == CARPENTRY_FACT
    assert world.agents[0].knowledge.knows(CARPENTRY_FACT)


def test_bootstrap_uses_joinery_technology_fact() -> None:
    """Discovered joinery bootstraps through the generic tech fact mapping."""
    prior = Knowledge(
        facts=frozenset(
            {
                FIRE_FACT,
    ...
    world, gains = bootstrap_discovered_knowledge(with_joinery)
    assert len(gains) == 1
    assert gains[0].fact == JOINERY_FACT
    assert world.agents[0].knowledge.knows(JOINERY_FACT)


def test_bootstrap_uses_cabinetry_technology_fact() -> None:
    """Discovered cabinetry bootstraps through the generic tech fact mapping."""
    prior = Knowledge(
        facts=frozenset(
            {
                FIRE_FACT,
    ...
    world, gains = bootstrap_discovered_knowledge(with_cabinetry)
    assert len(gains) == 1
    assert gains[0].fact == CABINETRY_FACT
    assert world.agents[0].knowledge.knows(CABINETRY_FACT)


def test_bootstrap_uses_ceramics_technology_fact() -> None:
    """Discovered ceramics bootstraps through the generic tech fact mapping."""
    prior = Knowledge(
        facts=frozenset(
            {
                FIRE_FACT,
    ...
    world, gains = bootstrap_discovered_knowledge(with_ceramics)
    assert len(gains) == 1
    assert gains[0].fact == CERAMICS_FACT
    assert world.agents[0].knowledge.knows(CERAMICS_FACT)


def test_bootstrap_uses_glazing_technology_fact() -> None:
    """Discovered glazing bootstraps through the generic tech fact mapping."""
    prior = Knowledge(
        facts=frozenset(
            {
                FIRE_FACT,
    ...
    world, gains = bootstrap_discovered_knowledge(with_glazing)
    assert len(gains) == 1
    assert gains[0].fact == GLAZING_FACT
    assert world.agents[0].knowledge.knows(GLAZING_FACT)


def test_bootstrap_uses_porcelain_technology_fact() -> None:
    """Discovered porcelain bootstraps through the generic tech fact mapping."""
    prior = Knowledge(
        facts=frozenset(
            {
                FIRE_FACT,
    ...
    world, gains = bootstrap_discovered_knowledge(with_porcelain)
    assert len(gains) == 1
    assert gains[0].fact == PORCELAIN_FACT
...
def test_bootstrap_uses_optics_technology_fact() -> None:
    """Discovered optics bootstraps through the generic tech fact mapping."""
    prior = Knowledge(
        facts=frozenset(
            {
                FIRE_FACT,
    ...
    world, gains = bootstrap_discovered_knowledge(with_optics)
    assert len(gains) == 1
    assert gains[0].fact == OPTICS_FACT
    assert world.agents[0].knowledge.knows(OPTICS_FACT)


def test_bootstrap_uses_crystal_technology_fact() -> None:
    """Discovered crystal bootstraps through the generic tech fact mapping."""
    prior = Knowledge(
        facts=frozenset(
            {
                FIRE_FACT,
    ...
    for technology in (
        ...
        assert updated is not None
        current = updated
    with_crystal = discover_technology(current, CAMP_CRYSTAL.technology_id)
    assert with_crystal is not None
    world, gains = bootstrap_discovered_knowledge(with_crystal)
    assert len(gains) == 1
    assert gains[0].fact == CRYSTAL_FACT
    assert world.agents[0].knowledge.knows(CRYSTAL_FACT)
...
def test_census_knowledge_counts_coverage() -> None:
    ...
    assert snap.smithing_knower_count == 0
    assert snap.toolmaking_knower_count == 0
    assert snap.carpentry_knower_count == 0
    assert snap.joinery_knower_count == 0
    assert snap.cabinetry_knower_count == 0
    ...
    assert snap.optics_knower_count == 0
    assert snap.crystal_knower_count == 0
