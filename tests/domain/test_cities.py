...
def test_census_cities_counts_residents() -> None:
    ...
    assert snap.active_guildhall_count == 0
    assert snap.active_pottery_town_count == 0
    assert snap.active_kiln_quarter_count == 0
    assert snap.active_glassworks_count == 0
    assert snap.active_crystal_quarter_count == 0
...
def test_create_entrepot_under_camp_government() -> None:
    """Entrepots may share a government with the capital on a distinct seat."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
    ...
    created = create_city(world, entrepot)
    assert created is not None
    assert city_by_id(created, 1) is not None
    assert city_by_id(created, 1).kind is CityKind.ENTREPOT  # type: ignore[union-attr]
    assert city_by_id(created, 1).is_capital is False  # type: ignore[union-attr]
    assert entrepots_for(created, CAMP_GOVERNMENT.government_id.value) == (entrepot,)
    snap = census_cities(created)
    assert snap.active_settlement_count == 1
    assert snap.active_entrepot_count == 1
    assert snap.capital_count == 1
    assert snap.city_count == 2


def test_create_rejects_capital_entrepot() -> None:
    """Entrepots cannot be capitals."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_set_capital_rejects_entrepot() -> None:
    """set_capital cannot promote an entrepot to capital."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_world_rejects_capital_entrepot() -> None:
    """World validation rejects entrepots flagged as capital."""
    with pytest.raises(ValidationError):
        World(
            config=SimulationConfig(agent_count=1, seed=1),
            locations=default_world_map()[:2],
...
def test_create_farmstead_under_camp_government() -> None:
    """Farmsteads may share a government with the capital on a distinct seat."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
    ...
    created = create_city(world, farmstead)
    assert created is not None
    assert city_by_id(created, 1) is not None
    assert city_by_id(created, 1).kind is CityKind.FARMSTEAD  # type: ignore[union-attr]
    assert city_by_id(created, 1).is_capital is False  # type: ignore[union-attr]
    assert farmsteads_for(created, CAMP_GOVERNMENT.government_id.value) == (farmstead,)
    snap = census_cities(created)
    assert snap.active_settlement_count == 1
    assert snap.active_farmstead_count == 1
    assert snap.capital_count == 1
    assert snap.city_count == 2


def test_create_rejects_capital_farmstead() -> None:
    """Farmsteads cannot be capitals."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_set_capital_rejects_farmstead() -> None:
    """set_capital cannot promote a farmstead to capital."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_world_rejects_capital_farmstead() -> None:
    """World validation rejects farmsteads flagged as capital."""
    with pytest.raises(ValidationError):
        World(
            config=SimulationConfig(agent_count=1, seed=1),
            locations=default_world_map()[:2],
...
def test_create_mill_town_under_camp_government() -> None:
    """Mill towns may share a government with the capital on a distinct seat."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
    ...
    created = create_city(world, mill_town)
    assert created is not None
    assert city_by_id(created, 1) is not None
    assert city_by_id(created, 1).kind is CityKind.MILL_TOWN  # type: ignore[union-attr]
    assert city_by_id(created, 1).is_capital is False  # type: ignore[union-attr]
    assert mill_towns_for(created, CAMP_GOVERNMENT.government_id.value) == (mill_town,)
    snap = census_cities(created)
    assert snap.active_settlement_count == 1
    assert snap.active_mill_town_count == 1
    assert snap.capital_count == 1
    assert snap.city_count == 2


def test_create_rejects_capital_mill_town() -> None:
    """Mill towns cannot be capitals."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_set_capital_rejects_mill_town() -> None:
    """set_capital cannot promote a mill town to capital."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_world_rejects_capital_mill_town() -> None:
    """World validation rejects mill towns flagged as capital."""
    with pytest.raises(ValidationError):
        World(
            config=SimulationConfig(agent_count=1, seed=1),
            locations=default_world_map()[:2],
...
def test_create_mining_camp_under_camp_government() -> None:
    """Mining camps may share a government with the capital on a distinct seat."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
    ...
    created = create_city(world, mining_camp)
    assert created is not None
    assert city_by_id(created, 1) is not None
    assert city_by_id(created, 1).kind is CityKind.MINING_CAMP  # type: ignore[union-attr]
    assert city_by_id(created, 1).is_capital is False  # type: ignore[union-attr]
    assert mining_camps_for(created, CAMP_GOVERNMENT.government_id.value) == (
        mining_camp,
    )
    snap = census_cities(created)
    assert snap.active_settlement_count == 1
    assert snap.active_mining_camp_count == 1
    assert snap.capital_count == 1
    assert snap.city_count == 2


def test_create_rejects_capital_mining_camp() -> None:
    """Mining camps cannot be capitals."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_set_capital_rejects_mining_camp() -> None:
    """set_capital cannot promote a mining camp to capital."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_world_rejects_capital_mining_camp() -> None:
    """World validation rejects mining camps flagged as capital."""
    with pytest.raises(ValidationError):
        World(
            config=SimulationConfig(agent_count=1, seed=1),
            locations=default_world_map()[:2],
...
def test_create_guildhall_under_camp_government() -> None:
    """Guildhalls may share a government with the capital on a distinct seat."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
    ...
    created = create_city(world, guildhall)
    assert created is not None
    assert city_by_id(created, 1) is not None
...
def test_create_rejects_capital_guildhall() -> None:
    """Guildhalls cannot be capitals."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_set_capital_rejects_guildhall() -> None:
    """set_capital cannot promote a guildhall to capital."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_world_rejects_capital_guildhall() -> None:
    """World validation rejects guildhalls flagged as capital."""
    with pytest.raises(ValidationError):
        World(
            config=SimulationConfig(agent_count=1, seed=1),
            locations=default_world_map()[:2],
...
def test_create_pottery_town_under_camp_government() -> None:
    """Pottery towns may share a government with the capital on a distinct seat."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
    ...
    created = create_city(world, pottery_town)
    assert created is not None
    assert city_by_id(created, 1) is not None
    assert city_by_id(created, 1).kind is CityKind.POTTERY_TOWN  # type: ignore[union-attr]
    assert city_by_id(created, 1).is_capital is False  # type: ignore[union-attr]
    assert pottery_towns_for(created, CAMP_GOVERNMENT.government_id.value) == (
        pottery_town,
    )
    snap = census_cities(created)
    assert snap.active_settlement_count == 1
    assert snap.active_pottery_town_count == 1
    assert snap.capital_count == 1
    assert snap.city_count == 2


def test_create_rejects_capital_pottery_town() -> None:
    """Pottery towns cannot be capitals."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_set_capital_rejects_pottery_town() -> None:
    """set_capital cannot promote a pottery town to capital."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_world_rejects_capital_pottery_town() -> None:
    """World validation rejects pottery towns flagged as capital."""
    with pytest.raises(ValidationError):
        World(
            config=SimulationConfig(agent_count=1, seed=1),
            locations=default_world_map()[:2],
...
def test_create_kiln_quarter_under_camp_government() -> None:
    """Kiln quarters may share a government with the capital on a distinct seat."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
    ...
    created = create_city(world, kiln_quarter)
    assert created is not None
    assert city_by_id(created, 1) is not None
    assert city_by_id(created, 1).kind is CityKind.KILN_QUARTER  # type: ignore[union-attr]
    assert city_by_id(created, 1).is_capital is False  # type: ignore[union-attr]
    assert kiln_quarters_for(created, CAMP_GOVERNMENT.government_id.value) == (
        kiln_quarter,
    )
    snap = census_cities(created)
    assert snap.active_settlement_count == 1
    assert snap.active_kiln_quarter_count == 1
    assert snap.capital_count == 1
    assert snap.city_count == 2


def test_create_rejects_capital_kiln_quarter() -> None:
    """Kiln quarters cannot be capitals."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_set_capital_rejects_kiln_quarter() -> None:
    """set_capital cannot promote a kiln quarter to capital."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_world_rejects_capital_kiln_quarter() -> None:
    """World validation rejects kiln quarters flagged as capital."""
    with pytest.raises(ValidationError):
        World(
            config=SimulationConfig(agent_count=1, seed=1),
            locations=default_world_map()[:2],
...
def test_create_glassworks_under_camp_government() -> None:
    """Glassworks may share a government with the capital on a distinct seat."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
    ...
    created = create_city(world, glassworks)
    assert created is not None
    assert city_by_id(created, 1) is not None
...
def test_create_rejects_capital_glassworks() -> None:
    """Glassworks cannot be capitals."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_set_capital_rejects_glassworks() -> None:
    """set_capital cannot promote a glassworks to capital."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_world_rejects_capital_glassworks() -> None:
    """World validation rejects glassworks flagged as capital."""
    with pytest.raises(ValidationError):
        World(
            config=SimulationConfig(agent_count=1, seed=1),
            locations=default_world_map()[:2],
...
def test_create_crystal_quarter_under_camp_government() -> None:
    """Crystal quarters may share a government with the capital on a distinct seat."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
    ...
    crystal_quarter = City.create(
        1,
        CAMP_GOVERNMENT.government_id.value,
        1,
        "Crystal Quarter",
        CityKind.CRYSTAL_QUARTER,
    )
    created = create_city(world, crystal_quarter)
    assert created is not None
    assert city_by_id(created, 1) is not None
    assert city_by_id(created, 1).kind is CityKind.CRYSTAL_QUARTER  # type: ignore[union-attr]
    assert city_by_id(created, 1).is_capital is False  # type: ignore[union-attr]
    assert crystal_quarters_for(created, CAMP_GOVERNMENT.government_id.value) == (
        crystal_quarter,
    )
    snap = census_cities(created)
    assert snap.active_settlement_count == 1
    assert snap.active_crystal_quarter_count == 1
    assert snap.capital_count == 1
    assert snap.city_count == 2


def test_create_rejects_capital_crystal_quarter() -> None:
    """Crystal quarters cannot be capitals."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_set_capital_rejects_crystal_quarter() -> None:
    """set_capital cannot promote a crystal quarter to capital."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map(),
        governments=(CAMP_GOVERNMENT,),
...
def test_world_rejects_capital_crystal_quarter() -> None:
    """World validation rejects crystal quarters flagged as capital."""
    with pytest.raises(ValidationError):
        World(
            config=SimulationConfig(agent_count=1, seed=1),
            locations=default_world_map()[:2],
