...
def test_default_innovations_seed_hearth_through_facet() -> None:
    ...
    assert CAMP_GLAZE.kind is InnovationKind.GLAZE
    assert CAMP_GLAZE.active is False
    assert CAMP_GLAZE.technology_id == CAMP_GLAZING.technology_id
    assert CAMP_KAOLIN.kind is InnovationKind.KAOLIN
    assert CAMP_KAOLIN.active is False
    assert CAMP_KAOLIN.technology_id == CAMP_PORCELAIN.technology_id
    assert CAMP_BLOWPIPE.kind is InnovationKind.BLOWPIPE
    assert CAMP_BLOWPIPE.active is False
    assert CAMP_BLOWPIPE.technology_id == CAMP_GLASSMAKING.technology_id
    assert CAMP_LENS.kind is InnovationKind.LENS
    assert CAMP_LENS.active is False
    assert CAMP_LENS.technology_id == CAMP_OPTICS.technology_id
    assert CAMP_FACET.kind is InnovationKind.FACET
    assert CAMP_FACET.active is False
    assert CAMP_FACET.technology_id == CAMP_CRYSTAL.technology_id
...
def test_activate_due_innovations_after_discovery() -> None:
    ...
    discovered = discover_technology(world, CAMP_GLAZING.technology_id)
    assert discovered is not None
    world, activations = activate_due_innovations(discovered)
    assert len(activations) == 1
    assert activations[0].kind is InnovationKind.GLAZE
    assert innovation_by_id(world, 32) is not None
    assert innovation_by_id(world, 32).active is True
    assert innovation_for_technology(world, 32) is not None

    discovered = discover_technology(world, CAMP_PORCELAIN.technology_id)
    assert discovered is not None
    world, activations = activate_due_innovations(discovered)
    assert len(activations) == 1
    assert activations[0].kind is InnovationKind.KAOLIN
    assert innovation_by_id(world, 33) is not None
    assert innovation_by_id(world, 33).active is True
    assert innovation_for_technology(world, 33) is not None

    discovered = discover_technology(world, CAMP_GLASSMAKING.technology_id)
    assert discovered is not None
    world, activations = activate_due_innovations(discovered)
    assert len(activations) == 1
    assert activations[0].kind is InnovationKind.BLOWPIPE
    assert innovation_by_id(world, 34) is not None
    assert innovation_by_id(world, 34).active is True
    assert innovation_for_technology(world, 34) is not None

    discovered = discover_technology(world, CAMP_OPTICS.technology_id)
    assert discovered is not None
    world, activations = activate_due_innovations(discovered)
    assert len(activations) == 1
    assert activations[0].kind is InnovationKind.LENS
    assert innovation_by_id(world, 35) is not None
    assert innovation_by_id(world, 35).active is True
    assert innovation_for_technology(world, 35) is not None

    discovered = discover_technology(world, CAMP_CRYSTAL.technology_id)
    assert discovered is not None
    world, activations = activate_due_innovations(discovered)
    assert len(activations) == 1
    assert activations[0].kind is InnovationKind.FACET
    assert innovation_by_id(world, 36) is not None
    assert innovation_by_id(world, 36).active is True
    assert innovation_for_technology(world, 36) is not None
...
def test_census_innovations_counts() -> None:
    ...
    assert snap.innovation_count == 37
    assert snap.active_count == 1
    assert snap.inactive_count == 36
    ...
    assert snap.active_glaze_count == 0
    assert snap.active_kaolin_count == 0
    assert snap.active_blowpipe_count == 0
    assert snap.active_lens_count == 0
    assert snap.active_facet_count == 0
