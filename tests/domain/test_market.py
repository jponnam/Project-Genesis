...
def test_sumptuary_reduces_market_fee_on_fill() -> None:
    """Active SUMPTUARY law at the market government discounts the fill fee."""
    government = Government.create(0, "Camp", 0, (0,))
    fee = Law.create(0, 0, "Stall Fee", LawKind.MARKET_FEE, flat_amount=2)
    sumptuary = Law.create(1, 0, "Camp Sumptuary", LawKind.SUMPTUARY)
    world = _world(
        _with_food(0, 1, money=0),
        _with_food(1, 0, money=4),
        governments=(government,),
...
def test_emporium_reduces_market_fee_on_fill() -> None:
    """Active emporium at the market seat discounts the fill fee by 1."""
    government = Government.create(0, "Camp", 0, (0,))
    fee = Law.create(0, 0, "Stall Fee", LawKind.MARKET_FEE, flat_amount=2)
    emporium = City.create(0, 0, 0, "Camp Emporium", CityKind.EMPORIUM)
    world = _world(
        _with_food(0, 1, money=0),
        _with_food(1, 0, money=4),
        governments=(government,),
...
def test_merchant_reduces_market_fee_on_fill() -> None:
    ...
    government = Government.create(0, "Camp", 0, (0,))
    fee = Law.create(0, 0, "Stall Fee", LawKind.MARKET_FEE, flat_amount=2)
    merchant = Institution.create(0, 0, 0, "Camp Merchant", InstitutionKind.MERCHANT)
    world = _world(
        _with_food(0, 1, money=0),
        _with_food(1, 0, money=4),
        governments=(government,),
...
def test_merchant_bureaucracy_and_harbor_stack_market_fee_on_fill() -> None:
    """Merchant, bureaucracy, and harbor discounts stack on market fills."""
    government = Government.create(0, "Camp", 0, (0,))
    fee = Law.create(0, 0, "Stall Fee", LawKind.MARKET_FEE, flat_amount=4)
    harbor = City.create(0, 0, 0, "Camp Harbor", CityKind.HARBOR)
    bureaucracy = Institution.create(
        0, 0, 0, "Camp Bureaucracy", InstitutionKind.BUREAUCRACY
    )
    ...
    world = _world(
        ...
        laws=(fee,),
        cities=(harbor,),
        institutions=(bureaucracy, merchant),
    )
