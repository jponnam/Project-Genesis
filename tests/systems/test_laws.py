...
def test_observe_emits_laws_observed_without_mutating_world() -> None:
    ...
    assert events[0].active_annealing_codes_count == 0
    assert events[0].active_crystal_codes_count == 0
...
def test_observe_emits_active_annealing_codes_count() -> None:
    """observe reports active ANNEALING_CODES laws in LawsObserved."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        governments=(Government.create(0, "Camp", 0, (0,)),),
        laws=(Law.create(0, 0, "Camp Annealing Codes", LawKind.ANNEALING_CODES),),
        agents=(Agent.create(agent_id=0, name="A"),),
    )
    bus = EventBus()
    LawSystem().observe(world, bus=bus)
    events = [event for event in bus.history if isinstance(event, LawsObserved)]
    assert len(events) == 1
    assert events[0].active_annealing_codes_count == 1


def test_observe_emits_active_crystal_codes_count() -> None:
    """observe reports active CRYSTAL_CODES laws in LawsObserved."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=(CAMP_LOCATION,),
        governments=(Government.create(0, "Camp", 0, (0,)),),
        laws=(Law.create(0, 0, "Camp Crystal Codes", LawKind.CRYSTAL_CODES),),
        agents=(Agent.create(agent_id=0, name="A"),),
    )
    bus = EventBus()
    LawSystem().observe(world, bus=bus)
    events = [event for event in bus.history if isinstance(event, LawsObserved)]
    assert len(events) == 1
    assert events[0].active_crystal_codes_count == 1
