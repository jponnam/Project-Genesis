...
def test_observe_emits_without_mutating_world() -> None:
    ...
    assert len(events) == 1
    assert events[0].technology_count == 37
    ...
    assert events[0].discovered_forestry_count == 0
    assert events[0].discovered_textiles_count == 0
    assert events[0].discovered_dyeing_count == 0
    assert events[0].discovered_tanning_count == 0
    assert events[0].discovered_mining_count == 0
    assert events[0].discovered_smithing_count == 0
    assert events[0].discovered_toolmaking_count == 0
    assert events[0].discovered_carpentry_count == 0
    assert events[0].discovered_joinery_count == 0
    assert events[0].discovered_cabinetry_count == 0
    assert events[0].discovered_ceramics_count == 0
    assert events[0].discovered_glazing_count == 0
    assert events[0].discovered_porcelain_count == 0
    assert events[0].discovered_glassmaking_count == 0
    assert events[0].discovered_optics_count == 0
    assert events[0].discovered_crystal_count == 0
    assert events[0].locked_count == 35
