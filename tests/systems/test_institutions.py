...
def test_observe_emits_institutions_observed_without_mutating_world() -> None:
    ...
    assert events[0].active_woodcutter_count == 0
    assert events[0].active_joiner_count == 0
    assert events[0].active_carver_count == 0
    assert events[0].active_potter_count == 0
    assert events[0].active_glazer_count == 0
    assert events[0].active_tilewright_count == 0
    assert events[0].active_glassblower_count == 0
    assert events[0].active_lensmaker_count == 0
    assert events[0].active_optician_count == 0
...
def test_observe_emits_active_health_institution_counts() -> None:
    ...
    world = World(
        ...
        institutions=(
            ...
            Institution.create(24, 0, 0, "Camp Glazer", InstitutionKind.GLAZER),
            Institution.create(25, 0, 0, "Camp Tilewright", InstitutionKind.TILEWRIGHT),
            Institution.create(
                26, 0, 0, "Camp Glassblower", InstitutionKind.GLASSBLOWER
            ),
            Institution.create(27, 0, 0, "Camp Lensmaker", InstitutionKind.LENSMAKER),
            Institution.create(28, 0, 0, "Camp Optician", InstitutionKind.OPTICIAN),
        ),
    ...
    assert events[0].active_woodcutter_count == 1
    assert events[0].active_joiner_count == 1
    assert events[0].active_carver_count == 1
    assert events[0].active_potter_count == 1
    assert events[0].active_glazer_count == 1
    assert events[0].active_tilewright_count == 1
    assert events[0].active_glassblower_count == 1
    assert events[0].active_lensmaker_count == 1
    assert events[0].active_optician_count == 1
