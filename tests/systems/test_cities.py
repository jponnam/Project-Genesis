...
def test_observe_reports_active_kiln_quarter_count() -> None:
    """observe publishes the active kiln quarter city count."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map()[:2],
        governments=(Government.create(0, "Camp", 0, (0, 1)),),
        cities=(
            City.create(0, 0, 0, "Camp", CityKind.SETTLEMENT, is_capital=True),
            City.create(1, 0, 1, "Kiln Quarter", CityKind.KILN_QUARTER),
        ),
        agents=(Agent.create(agent_id=0, name="A", location_id=1),),
    )
    bus = EventBus()
    ...
    events = [event for event in bus.history if isinstance(event, CitiesObserved)]
    assert len(events) == 1
    assert events[0].active_kiln_quarter_count == 1
    assert events[0].total_residents == 1


def test_observe_reports_active_glassworks_count() -> None:
    """observe publishes the active glassworks city count."""
    world = World(
        config=SimulationConfig(agent_count=1, seed=1),
        locations=default_world_map()[:2],
        governments=(Government.create(0, "Camp", 0, (0, 1)),),
        cities=(
            City.create(0, 0, 0, "Camp", CityKind.SETTLEMENT, is_capital=True),
            City.create(1, 0, 1, "Glassworks", CityKind.GLASSWORKS),
        ),
        agents=(Agent.create(agent_id=0, name="A", location_id=1),),
    )
    bus = EventBus()
