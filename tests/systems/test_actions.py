...
def test_rest_uses_active_infirmary_restore_bonus() -> None:
    """REST through ActionExecutor includes active infirmary city seat bonus."""
    agent = Agent.create(
        agent_id=0,
        name="A",
        location_id=1,
...
def test_eat_uses_active_zoning_restore_bonus() -> None:
    ...
    updated = ActionExecutor().execute(world, _choice(0, ActionKind.EAT))
...
def test_eat_uses_active_land_tenure_restore_bonus() -> None:
    """EAT through ActionExecutor includes subject-scoped land-tenure bonus."""
    agent = Agent.create(
        agent_id=0,
        name="A",
        needs=Needs(food=0.4, water=1.0, energy=1.0, social=1.0, safety=1.0),
...
def test_drink_action_uses_apothecary_bonus() -> None:
    """DRINK restore includes active apothecary seat bonuses."""
    agent = Agent.create(
        agent_id=0,
        name="A",
        needs=Needs(food=1.0, water=0.4, energy=1.0, social=1.0, safety=1.0),
...
def test_produce_uses_customs_effective_energy_cost() -> None:
    """PRODUCE through ActionExecutor applies customs subject discount."""
...
def test_produce_uses_safety_codes_effective_energy_cost() -> None:
    """PRODUCE through ActionExecutor applies safety-codes subject discount."""
    rations_energy_cost = 0.05
    agent = Agent.create(agent_id=0, name="A").model_copy(
        update={
            "inventory": Inventory(
    ...
    assert updated.agents[0].inventory.quantity("rations") == 1
    assert updated.agents[0].needs.energy == pytest.approx(
        0.05 - (rations_energy_cost - SAFETY_CODES_PRODUCE_ENERGY_DISCOUNT)
    )
