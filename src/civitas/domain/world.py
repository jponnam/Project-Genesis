...
class World(BaseModel):
    ...
    def world_must_be_consistent(self) -> Self:
        ...
        active_curriculum_govs: set[int] = set()
        ...
        for law in self.laws:
            ...
            if law.active and law.kind == LawKind.CURRICULUM:
                gov_value = law.government_id.value
                if gov_value in active_curriculum_govs:
                    msg = "at most one active CURRICULUM law per government"
                    raise ValueError(msg)
                active_curriculum_govs.add(gov_value)
            if law.active and law.kind == LawKind.CALENDAR:
                gov_value = law.government_id.value
                if gov_value in active_calendar_govs:
                    msg = "at most one active CALENDAR law per government"
                    raise ValueError(msg)
                active_calendar_govs.add(gov_value)
            if law.active and law.kind == LawKind.ETHICS:
                gov_value = law.government_id.value
                if gov_value in active_ethics_govs:
                    msg = "at most one active ETHICS law per government"
                    raise ValueError(msg)
                active_ethics_govs.add(gov_value)
            if law.active and law.kind == LawKind.ASSEMBLY:
                gov_value = law.government_id.value
                if gov_value in active_assembly_govs:
            ...
            if law.active and law.kind == LawKind.QUARANTINE:
                gov_value = law.government_id.value
                if gov_value in active_quarantine_govs:
                    msg = "at most one active QUARANTINE law per government"
                    raise ValueError(msg)
                active_quarantine_govs.add(gov_value)
            if law.active and law.kind == LawKind.BUILDING_CODES:
                gov_value = law.government_id.value
                if gov_value in active_building_codes_govs:
            ...
            if law.active and law.kind == LawKind.PASSAGE:
                gov_value = law.government_id.value
                if gov_value in active_passage_govs:
                    msg = "at most one active PASSAGE law per government"
                    raise ValueError(msg)
                active_passage_govs.add(gov_value)
            if law.active and law.kind == LawKind.CUSTOMS:
                gov_value = law.government_id.value
                if gov_value in active_customs_govs:
                    msg = "at most one active CUSTOMS law per government"
                    raise ValueError(msg)
                active_customs_govs.add(gov_value)
            if law.active and law.kind == LawKind.LAND_TENURE:
                gov_value = law.government_id.value
                if gov_value in active_land_tenure_govs:
            ...
            if law.active and law.kind == LawKind.LABOR:
                gov_value = law.government_id.value
                if gov_value in active_labor_govs:
                    msg = "at most one active LABOR law per government"
                    raise ValueError(msg)
                active_labor_govs.add(gov_value)
            if law.active and law.kind == LawKind.SUMPTUARY:
                gov_value = law.government_id.value
                if gov_value in active_sumptuary_govs:
                    msg = "at most one active SUMPTUARY law per government"
                    raise ValueError(msg)
                active_sumptuary_govs.add(gov_value)
            if law.active and law.kind == LawKind.MINERAL_RIGHTS:
                gov_value = law.government_id.value
                if gov_value in active_mineral_rights_govs:
            ...
            if law.active and law.kind == LawKind.TIMBER_RIGHTS:
                gov_value = law.government_id.value
                if gov_value in active_timber_rights_govs:
                    msg = "at most one active TIMBER_RIGHTS law per government"
                    raise ValueError(msg)
                active_timber_rights_govs.add(gov_value)
            ...
            if law.active and law.kind == LawKind.CLAY_CODES:
                gov_value = law.government_id.value
                if gov_value in active_clay_codes_govs:
                    msg = "at most one active CLAY_CODES law per government"
                    raise ValueError(msg)
                active_clay_codes_govs.add(gov_value)
            if law.active and law.kind == LawKind.ANNEALING_CODES:
                gov_value = law.government_id.value
                if gov_value in active_annealing_codes_govs:
