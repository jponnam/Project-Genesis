...
FORESTRY_FACT: str = TechnologyKind.FORESTRY.value
TEXTILES_FACT: str = TechnologyKind.TEXTILES.value
...
CERAMICS_FACT: str = TechnologyKind.CERAMICS.value
GLAZING_FACT: str = TechnologyKind.GLAZING.value
...
def census_knowledge(world: World) -> KnowledgeCensus:
    ...
    surveying_knowers = sum(
        1 for agent in living if agent.knowledge.knows(SURVEYING_FACT)
    )
    navigation_knowers = sum(
        1 for agent in living if agent.knowledge.knows(NAVIGATION_FACT)
    )
    ...
    forestry_knowers = sum(
        1 for agent in living if agent.knowledge.knows(FORESTRY_FACT)
    )
    ...
    toolmaking_knowers = sum(
        1 for agent in living if agent.knowledge.knows(TOOLMAKING_FACT)
    )
    ...
    return KnowledgeCensus(
        ...
        forestry_knower_count=forestry_knowers,
        textiles_knower_count=textiles_knowers,
        ...
        ceramics_knower_count=ceramics_knowers,
        glazing_knower_count=glazing_knowers,
