...
Milestones 4 and 9, Phase 13 Milestones 4 and 9, Phase 14
Milestones 4 and 9, and Phase 15 Milestones 4 and 9. Infrastructure pieces
...
``CLAY_PIT``, ``GLASSHOUSE``, and ``LEHR`` are available via free create or paid
construction.
...
SCAFFOLD wood gather, LUMBER_YARD wood gather (stacking with sawmill/
coppice society-wide, scaffold/woodcutter seats, pastoral city, and the
timber rights subject bonus), DITCH water gather (stacking with
...
INFRASTRUCTURE_BUILD_COSTS: dict[InfrastructureKind, int] = {
    ...
    InfrastructureKind.LEHR: DEFAULT_LEHR_BUILD_COST,
}
...
class InfrastructureCensus(BaseModel):
    ...
    active_lehr_count: NonNegativeInt = 0
...
def census_infrastructure(world: World) -> InfrastructureCensus:
    ...
    active_ditches = sum(1 for item in active if item.kind is InfrastructureKind.DITCH)
    active_terraces = sum(
    ...
    active_lehrs = sum(1 for item in active if item.kind is InfrastructureKind.LEHR)
    return InfrastructureCensus(
        ...
        active_lehr_count=active_lehrs,
    )
