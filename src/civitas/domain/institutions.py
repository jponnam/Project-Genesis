...
Milestone 3 glassblowers, Milestone 6 lensmakers, and Milestone 8 opticians.
...
glassblowers, lensmakers, and opticians
coexist; this
...
def census_institutions(world: World) -> InstitutionCensus:
    ...
    active_smelters = sum(
        1 for institution in active if institution.kind is InstitutionKind.SMELTER
    )
    active_smiths = sum(
    ...
    active_joiners = sum(
        1 for institution in active if institution.kind is InstitutionKind.JOINER
    )
    active_carvers = sum(
        1 for institution in active if institution.kind is InstitutionKind.CARVER
    )
    active_potters = sum(
    ...
    active_tilewrights = sum(
        1 for institution in active if institution.kind is InstitutionKind.TILEWRIGHT
    )
    ...
    active_opticians = sum(
        1 for institution in active if institution.kind is InstitutionKind.OPTICIAN
    )
    ...
    return InstitutionCensus(
        ...
        active_optician_count=active_opticians,
