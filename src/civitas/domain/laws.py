...
codes, and loom.
Active ``ANNEALING_CODES`` statutes grant living subjects -0.02 PRODUCE
energy cost (Phase 20 M2), stacking with guild, workshop, weaver, smelter,
joiner, foundry, fulling mill, forge works, sawpit, mill town, ironworks,
guildhall, tannery, bellows, lathe, plane, dovetail, kiln, glaze, kaolin,
clay pit, kiln yard, abacus, pulley, customs, labor, safety codes, firing
...
def active_annealing_codes_law(
    world: World,
    government_id: GovernmentId | int,
) -> Law | None:
    """Return the active annealing-codes statute for ``government_id``, if any.

    When multiple active ``ANNEALING_CODES`` laws exist (should not under
    uniqueness rules), the lowest ``law_id`` wins.
    """
    for law in active_laws(world, government_id):
        if law.kind == LawKind.ANNEALING_CODES:
            return law
    return None


def annealing_codes_produce_discount_for(world: World, agent: Agent) -> float:
    """Return -0.02 PRODUCE cost when ``agent`` is under active ANNEALING_CODES.
    ...
    if government is None:
        return 0.0
    if active_annealing_codes_law(world, government.government_id) is None:
        return 0.0
